                ...
                <div class="row">
                    <div class="col">
                        <div class="card shadow mb-3">
                            ...
                            <div class="card-body">
                                <form method="POST" action="{{route('agent.profile.update',['agent'=>$agent->id])}}">
                                    @csrf
                                    @method('put')
                                    <div class="form-row">
                                        <div class="col">
                                            <div class="form-group"><label for="firstname"><strong>First Name</strong></label><input class="form-control @error('firstname') is-invalid @enderror" type="text" placeholder="First Name" value="{{old('firstname') ? old('firstname') : $agent->firstname}}" name="firstname"></div>
                                        </div>
                                        <div class="col">
                                            <div class="form-group"><label for="lastname"><strong>Last Name</strong></label><input class="form-control @error('lastname') is-invalid @enderror" type="text" placeholder="Last Name" name="lastname" value="{{old('lastname') ? old('lastname') : $agent->lastname}}"></div>
                                        </div>
                                    </div>
                                    <div class="form-row">
                                        <div class="col">
                                            <div class="form-group"><label for="phone"><strong>Phone</strong></label><input class="form-control @error('phone') is-invalid @enderror" type="tel" placeholder="" name="" value="{{old('phone') ? old('phone') : $agent->phone}}"></div>
                                        </div>
                                        <div class="col">
                                            <div class="form-group"><label for="email"><strong>Email Address</strong></label><input class="form-control @error('email') is-invalid @enderror" type="email" placeholder="user@example.com" name="email" value="{{old('email') ? old('email') : $agent->email}}"></div>
                                        </div>
                                    </div>
                        ...
                        <div class="card shadow">
                            ...
                            <div class="card-body">
                                <form action="{{route('agent.profile.update',['agent'=>$agent->id])}}" method="POST">
                                    @csrf
                                    @method('put')
                                    <div class="form-group"><label for="address"><strong>Address</strong></label><input class="form-control @error('address') is-invalid @enderror" type="text" placeholder="eg. Sunset Blvd, 38" name="address" value="{{old('address') ? old('address') : $agent->address}}"></div>
                                    <div class="form-row">
                                        <div class="col">
                                            <div class="form-group">
                                                <label for="country"><strong>Country</strong></label>
                                                <select name="country" id="country" class="form-control @error('country') is-invalid @enderror">
                                                    @if(!$countries->isEmpty() && is_null($agent->country_id))
                                                        <option value="">Select a country</option>
                                                    @endif
                                                    @forelse ($countries as $country)
                                                        <option value="{{$country->id}}" @if($agent->country_id == $country->id) selected @elseif(old('country') == $country->id) selected @endif>{{ucwords($country->name)}}</option>
                                                    @empty
                                        ...
                                        <div class="col">
                                            <div class="form-group"><label for="state"><strong>State</strong></label>
                                                <select name="state" id="state" class="form-control @error('state') is-invalid @enderror">
                                                    @if($agent->state_id)
                                                    <option value="{{$agent->state_id}}">{{ucwords($agent->state->name)}}</option>
                                                    @endif
                                                </select>
                                            </div>
                                        </div>
                                        <div class="col">
                                            <div class="form-group"><label for="city"><strong>City</strong></label>
                                                <select name="city" id="city" class="form-control @error('city') is-invalid @enderror">
                                                    @if($agent->city_id)
                                                    <option value="{{$agent->city_id}}">{{ucwords($agent->city->name)}}</option>
                                                    @endif
                                                </select>
                                            </div>
                                        </div>
